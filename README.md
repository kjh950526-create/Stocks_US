...
## ★★★ Claude 반복실수 자기교정 체크리스트 (매 브리핑/응답 전 self-check, 반복지적 방지) ★★★
**Jeonghun이 같은 실수를 반복 지적하게 만들지 말 것. 아래는 이미 여러번 지적된 실수들 — 매번 스스로 점검:**
1. **매크로 "왜" 누락** (7/23,8/4,8/5 지적) → 모든 브리핑 web_search 먼저.
2. **팩터 내 리더 로테이션 누락** (8/10 지적) → RS63 재계산으로 각 팩터 현재 리더 확인.
3. **관성으로 비리더/구관심주 리스트 유지** (8/10 지적: FTNT/CRWD/DDOG를 리더 아닌데 "따로 챙김") →
   진입대상은 리더만. "우리가 봐온 종목"이라는 이유로 올리지 말 것.
4. **엔진 라벨 무비판 수용** (8/6 지적: SNOW "flag"라 했으나 실제 확장중) → 엔진 state는 참고, 차트
   구조를 직접 재판단.
5. **스토리 부풀리기** (8/6 지적: AMD 진입회피는 과장, 후보였던 적 없음) → "규칙이 이만큼 지켰다"
   과대포장 금지, 실제 케이스만.
6. **사용자 밀어붙임에 동조** (8/5 지적) → 데이터가 상대편일 때만 의견변경, 근거없는 낙관 거부.
7. **zone/가격을 점으로 취급** (8/5,8/10 반복지적) → zone은 선. 0.3ADR 이내는 "도달".
8. **20선 수렴을 응집 조건으로 오적용** (8/17 지적) → 진입 트리거는 **10선**(§239-244, §357). 20선
   수렴 대기는 "안 사겠다"와 같음(강손은 20선 붙는 데 16일+). 20선은 손절/트레일/죽음판별용이지
   진입조건 아님. "20선 멀어서 응집 미완"이라 말하지 말 것.
9. **"초강손" 오분류** (8/21 지적) → 초강손 = 응집없이 쭉 오르는 종목(RS1위+신고가+응집X). **응집중인
   종목(flag)을 초강손이라 부르면 모순.** 태그 전 반드시 확인: 최근 15일 밴드폭 수축=응집이면 초강손
   아니라 "급등후 첫응집(갈래A)". PLTR($172-180 flag)을 초강손이라 한 게 오류. 초강손/갈래A/저점반등을
   섞지 말 것.
10. **레짐 무시하고 "버티는 종목" 워치 추가** (8/21 지적) → 칼질/조정장에 "성장주 전멸 속 버티는 종목"을
    넣는 건 부록19 위반(칼질장=저점반등, 추세장=초강손 추격). "버틴다"는 진입사유 아님. 워치 추가 전
    현 레짐에 맞는 유형인지 확인. 폐기한 "1위/강한거 추격" 습관 재발 주의.
**★이 파일에 실수를 기록하는 이유 = 반복 방지. 새 실수 지적받으면 여기 추가하고, 매 응답 전 훑을 것.**
...
## ★★능동적 사고 원칙 (Claude 필독, 2026-08-26 Jeonghun 강력 지적) ★★
Jeonghun이 원하는 것은 **능동성**이다. 답변 방식을 일일이 지시받지 않아도 스스로 논리적으로 생각하라.
- **사용자의 구체적 전제를 백테스트 조건으로 그대로 옮겨라.** 뭉뚱그린 일반 검증(예: "갭업=+5%이상
  하나로" → 50% 동전던지기)은 사용자가 세운 맥락(원래1등+밀림+실적촉매)을 버리는 게으른 분석이다.
- **결과 수치를 그대로 읊지 말고 "왜 그 수치인가"를 따져라.** 50%가 나오면 기준이 엉성한 건 아닌지
  의심하라. (실제로 "갭업 +5%↑ 뭉침"이 강갭 6-12% 나쁜구간에 휩쓸려 왜곡된 사례 있음.)
- **일반론으로 후퇴하지 마라.** 사용자가 준 맥락·전제가 핵심이다. "무조건 갭업=추격, 갭업=50%" 같은
  수동적 사고 금지. 조건을 겹겹이 넣어 정밀하게 검증하라.
- **먼저 생각하고 설계하라.** 사용자가 검증 방법까지 다 불러주게 하지 마라.

## ★검증 설계 원칙 ★
백테스트 전 반드시: (1)사용자 전제를 조건으로 분해(원래순위/최근변화/촉매/펀더 등 다층), (2)강도·구간
세분(1%와 8% 갭업은 다르다), (3)비교군 설정(전제 조합 vs 랜덤), (4)표본수 확인(적으면 방향성만),
(5)왜 그 수치인지 해석. 뭉뚱그리면 실제 엣지가 노이즈에 묻힌다.

## ★★2층 팩터 분석 필수 (매 브리핑, 절대 생략금지, 2026-08-31 Jeonghun 지시) ★★
팩터는 항상 **2층**으로 해석. 하나라도 빼먹지 말것:
- **대분류(대군 로테이션)**: 소프트웨어 대군(cyber/ai_sw/observability, 상호상관 0.47~0.72) vs 하드웨어
  (반도체). 어느 대군으로 돈이 흐르나 = 큰 방향.
- **소분류(대군 내 리더)**: 그 대군 안에서 cyber/ai_sw 등 누가 1등(번갈아 경쟁) = 종목 선택.
- 소분류 리더 복귀를 잡되 대군 부상과 겹치면 최강(CRWD). 소분류 1등 밀려도 대군 리딩이면 그 대군 다른
  소분류가 받음. 부록42-43.
- **매 브리핑 (1)대군 로테이션 (2)소분류 리더 둘다 명시.** 하나만 보면 "cyber인데 리딩은 ai_sw?" 혼란.
