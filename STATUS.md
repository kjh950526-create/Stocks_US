# ★★★ 실행 환경 (절대 헷갈리지 말것) ★★★
**IBKR 페이퍼 계좌. 정수주. ★하드스탑 가능(GTC 스탑주문)★.** CRWD도 BE 스탑 $203.97 GTC로 실제 청산됨.
"하드스탑 없다"는 옛 Toss 프랙셔널 기준 — 그 표현 다신 쓰지 말것. 스탑 주문 정상 작동함(단 갭다운은 스탑도
못막음 = BLZE, 이건 하드스탑 유무와 무관한 갭리스크).

# ★★★ 실제 성적 (2026-09-02, 정확히) ★★★
시작 ~$19,886 → 현재 $19,205 = **누적 -$681 (-3.4%). 4전 4패.**
- PANW -$286.97 / SNOW -$157.94 / BLZE -$186.14 / CRWD -$49.60. 합 -$680.65.
- 팩트: 4번 다 손실. 계속 짐. (이전에 Claude가 "-0.24%"라 한건 CRWD 개별손익을 계좌전체로 착각한 계산오류.)
- 검증단계 페이퍼. 최대손실 PANW -7.5%였고 나머지 소액 = 리스크관리는 작동. but 승리 아직 0.
...
## 시장 스냅샷 (2026-09-02 수 종가) — 2층 분석

**【대분류】** 소프트웨어 대군 리딩 유지되나 격차 급속축소(정규화 +3.94→2.97→**+1.51**, 3일연속 조정 동력
약화) vs 하드웨어 -0.85. **【소분류】 ★리더교체: ai_software(+1.89) > cyber(+1.37, 3일연속 급락 오늘-5.3%)
> obs(+0.29).** Jeonghun 며칠전 "cyber→ai_sw 자금이동" 관찰 실현.

- **★CRWD BE스탑 청산: $203.97, -$49.6 (-0.24%).** 진입$206.67→최고$231(+11.8%)→BE청산. 큰수익($422)
  놓쳤으나 **손실0 방어 = BE스탑 목적 정확 달성.** 포지션 0, 100% 현금.
- **★AVGO 실적 = 부록42 검증:** 매출$29.59B +86%YoY beat, EPS$3.32 beat, Q4가이던스+93% 강력. **그럼에도
  시간외 -6%.** = 지는대군(HW)은 강한실적도 못뜸. **어제 AVGO 진입안한 판단 정확.**
- 국면: QQQ709 +0.2%(어제-1.3%서 소폭반등) but 50선-0.1% 이탈유지, VIX15.2, 7/29후 25일차. 지수는버티나
  SW(리더)가 조정 = 리더로테이션/조정심화 갈림길.
- **★개선점(부록37 보완 필요): 큰돌파후 +2ADR 넘으면(어제 +11.8%) 절반익절 트리거($238=+3ADR) 안와도
  일부 익절 고려.** 하드스탑없는 환경선 +2ADR 도달만으로 일부 방어가 나음. 규칙대로 홀드했으나 시장조정
  겹쳐 BE까지 옴 — 프로세스는 맞았지만 트리거를 +3ADR 단일이 아니라 "+2ADR 도달+모멘텀꺾임"으로 넓힐것.
- 자책 아님(부록 결과로 프로세스 평가금지): BE가 손실0 방어했고, 홀드는 규칙대로였음.
